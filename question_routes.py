...
def get_questions_by_quiz(quiz_id):
    ...
    for q in quiz.questions:
        answers_list = []
        if q.answers:
            # Primeiro tenta interpretar como JSON (se foram salvas como tal)
            try:
                answers_list = json.loads(q.answers)
            except json.JSONDecodeError:
                # Se falhar, tenta dividir pela barra vertical (formato "D. Afonso Henriques|...")
                if '|' in q.answers:
                    answers_list = q.answers.split('|')
                else:
                    # Fallback para dados antigos salvos como string Python "['a', 'b']"
                    try:
                        answers_list = json.loads(q.answers.replace("'", '"'))
                    except:
                        answers_list = [] # Se tudo falhar, a lista fica vazia
...
def add_question(quiz_id):
    ...
    # Validação: Só o autor pode adicionar perguntas
    ...
    new_question = Question(
        ...
        answers=json.dumps(data['answers']), # Serializa para JSON
...
def update_question(question_id):
    ...
    if 'answers' in data:
        question.answers = json.dumps(data['answers']) # Serializa para JSON
